const SITE_IDS = [
  "COW-ND001", "COW-ND002", "COW-ND003", "COW-ND004", "COW-ND005", "COW-ND006",
  "COW-ND007", "COW-ND008", "COW-ND009", "COW-ND010", "COW-ND011", "COW-ND012",
  "COW-ND013", "COW-ND014", "COW-ND015", "COW-ND016", "COW-ND017", "COW-ND018",
  "COW-ND019", "COW-ND020", "COW-ND021", "COW-ND022", "COW-ND023", "COW-ND024",
] as const;
...
function rowsFromValues(values: unknown[][], headerRowIndex: number): StringRow[] {
  ...
  return values.slice(headerRowIndex + 1).map((rawRow) => {
    const row: StringRow = {};
...
function isInactiveStatus(value: string): boolean {
  return ["CLOSED", "CLEARED", "RESOLVED", "INACTIVE", "CANCELLED", "CANCELED"].includes(value);
}
...
function isoTimestamp(row: StringRow, directAliases: string[], dateAliases: string[], timeAliases: string[]): string | null {
...
function timestampFrom(row: StringRow, directAliases: string[], dateAliases: string[], timeAliases: string[]): string | null {
  return isoTimestamp(row, directAliases, dateAliases, timeAliases);
}
...
function powerAlarmRows(rows: StringRow[]): { rows: Record<string, unknown>[]; tickets: Set<string> } {
  ...
  const mapped = rows.flatMap((row) => {
    ...
    const alarm = firstValue(row, ["alarm", "power_alarm", "power alarm", "alarm_name", "alarm name", "alarm_description", "description"]);
    const status = statusValue(firstValue(row, ["status", "state", "alarm_status"]));
    if (!SITE_ID_SET.has(siteId) || !ttNumber || isNoAlarm(alarm) || isNoAlarm(ttNumber) || isNoAlarm(status)) return [];
    tickets.add(ttNumber);
    return [{
      site_id: siteId,
      tt_number: ttNumber,
      alarm: nullable(alarm),
      status,
      ...
      source_updated_at: timestampFrom(row, ["updated_at", "updated at", "last_updated"], ["updated_date", "updated date"], ["updated_time", "updated time"]),
...
function outageRows(rows: StringRow[]): { rows: Record<string, unknown>[]; tickets: Set<string> } {
  const tickets = new Set<string>();
  const mapped = rows.flatMap((row) => {
    const siteId = siteIdFrom(row);
    const ttNumber = firstValue(row, ["tt_number", "tt number", "ticket_number", "ticket number", "trouble_ticket", "tt"]);
    const outageType = firstValue(row, ["outage_type", "outage type", "oos_reason", "oos reason", "reason", "sir_type", "type"]);
    const status = statusValue(firstValue(row, ["status", "state", "sir_status"]));
    if (!SITE_ID_SET.has(siteId) || !ttNumber || isNoAlarm(ttNumber) || isNoAlarm(outageType) || isNoAlarm(status)) return [];
    tickets.add(ttNumber);
    return [{
      site_id: siteId,
      tt_number: ttNumber,
      outage_type: nullable(outageType),
      status,
...
async function sync(): Promise<SyncCounts> {
  ...
  const power = powerAlarmRows(rowsFromValues(powerValues, 0));
  const outages = outageRows(rowsFromValues(outageValues, 1));
  ...
  return {
    powerSourceRows: powerValues.length > 0 ? powerValues.length - 1 : 0,
    outageSourceRows: outageValues.length > 1 ? outageValues.length - 2 : 0,
    powerUpserted,
    outagesUpserted,
    powerDeactivated,
    outagesDeactivated,
